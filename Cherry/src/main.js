...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // The tabs-list itself is made scrollable via overflow-x:auto added inline
  // (avoids touching styles.css visual design).
  ...
  // Hide webkit scrollbar via a dynamic <style> tag so we don't touch styles.css
  const scrollStyle = document.createElement("style");
  scrollStyle.textContent = `#tabs-list::-webkit-scrollbar { display: none; }`;
  document.head.appendChild(scrollStyle);
  // Also make the tabs-container clip correctly
  ...
  require(['vs/editor/editor.main'], function () {
    ...
    // Set up initial model for tab 1
    ...
    tabsList.addEventListener("click", (e) => {
      ...
      if (close && tab) {
        ...
        // Tab "1" (Script #1) is permanently uncloseable
        if (id === "1") return;
        ...
        // Reset counter so next new tab resumes from the right number
        scriptCounter = document.querySelectorAll(".script-tab").length;

        // Always fall back to tab "1"
        switchTab("1");
        return;
      }
